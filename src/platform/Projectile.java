...
public abstract class Projectile extends ObjectWithColliders {
	...
	public Projectile(double x, double y, double xVel, double yVel, int hitboxWidth, int hitboxHeight,
			int framesLeftAlive, boolean[] canHitType, int damage, PGamePanel pgpRef) {
		super(x, y, xVel, yVel, hitboxWidth, hitboxHeight, pgpRef);
	...
	int shouldKill() {
		for (Platform p : getPgpRef().platforms) {
			...
			if (temp.contains(getTop()) || temp.contains(getBottom()) || temp.contains(getLeft())
					|| temp.contains(getRight())) {
				if (canHitWall) {
	...
	void drawObject(Graphics g, double xInc, double yInc) {
		...
		g2d.drawImage(image, (int) (getX() - image.getWidth() / 2 + xInc),
				(int) (getY() - image.getHeight() / 2 + yInc), null);
		g2d.dispose();
	}
}
