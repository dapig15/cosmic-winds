...
public class Arena extends JPanel {
    ...
    public void paintComponent(Graphics g) {
    	...
    	g.drawRect(player.getBody().getX1(), boundy - player.getBody().getY2(), 
    			player.getBody().getX2() - player.getBody().getX1(), player.getBody().getY2() - player.getBody().getY1());
    	g.setColor(Color.red);
    	g.drawRect(opponent.getBody().getX1(), boundy - opponent.getBody().getY2(), 
    			opponent.getBody().getX2() - opponent.getBody().getX1(), opponent.getBody().getY2() - opponent.getBody().getY1());
    	if (player.getCurrMove() != null) {
    		g.setColor(Color.blue);
    		g.drawRect(player.getAttack().getX1(), boundy - player.getAttack().getY2(), 
        			player.getAttack().getX2() - player.getAttack().getX1(), player.getAttack().getY2() - player.getAttack().getY1());
    	}
    	if (opponent.getCurrMove() != null) {
    		g.setColor(Color.red);
    		g.drawRect(opponent.getAttack().getX1(), boundy - opponent.getAttack().getY2(), 
    				opponent.getAttack().getX2() - opponent.getAttack().getX1(), opponent.getAttack().getY2() - opponent.getAttack().getY1());
    	}
    }
}